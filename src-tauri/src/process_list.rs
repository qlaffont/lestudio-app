...
use crate::{
    commands::{get_default_action, get_token, update_games_list},
    config::get_api_base,
    filepath::{get_game_list_path, get_local_game_list_path},
};
...
pub async fn periodically_send_process_list(window: &tauri::Window) {
    ...
    loop {
        ...
        window
            .emit(
                PROCESS_LIST_CHANNEL,
                Payload {
                    message: processes_string,
                },
            )
            .unwrap();
        ...
        for game in &game_list {
            for process in &processes_vec {
                if process
                    .to_lowercase()
                    .eq(game.processName.to_lowercase().as_str())
                {
        ...
        if let Some(game) = &detected_game {
            ...
            window
                .emit(
                    DETECTED_GAME_CHANNEL,
                    Payload {
                        message: serde_json::to_string(&game).unwrap(),
                    },
                )
                .unwrap();
        } else {
